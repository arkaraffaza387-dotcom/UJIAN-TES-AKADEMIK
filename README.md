...
<body class="bg-slate-50 font-sans min-h-screen">
    <div class="container mx-auto px-4 py-6 max-w-4xl">
        ...
        <section id="halaman-ujian" class="bg-white rounded-xl shadow p-6 mb-10">
            ...
            <div id="bagianAkun" class="hidden">
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" id="userMasuk" placeholder="Nama Pengguna" class="w-full border rounded p-2 mb-2">
                        <input type="password" id="passMasuk" placeholder="Kata Sandi" class="w-full border rounded p-2 mb-2">
                    ...
                    <div>
                        ...
                        <p class="text-xs mb-2">Kode Akses: <b>QRTGHS-2026</b></p>
                        <input type="text" id="userBaru" placeholder="Nama Pengguna Baru" class="w-full border rounded p-2 mb-2">
                        <input type="password" id="passBaru" placeholder="Kata Sandi Baru" class="w-full border rounded p-2 mb-2">
                        <input type="text" id="kodeAkses" placeholder="Masukkan Kode Akses" class="w-full border rounded p-2 mb-2 uppercase">
                        <button onclick="prosesDaftar()" class="w-full bg-aksen text-white py-2 rounded">Buat Akun</button>
                    </div>
                </div>
                <div class="mt-4 text-center">
                    <button onclick="kembaliPilihan()" class="text-sm text-slate-500">&larr; Kembali ke Pengaturan</button>
                </div>
            </div>
            ...
            <div id="bagianUjian" class="hidden">
                ...
                <button onclick="selesaikanUjian()" class="w-full bg-utama text-white py-2 rounded">✅ Selesaikan Ujian</button>
            </div>
            ...
            <div id="bagianSelesai" class="hidden text-center py-6">
                ...
                <h2 class="text-xl font-bold mb-2">Ujian Telah Selesai! Nilai tersimpan aman.</h2>
                <a href="#halaman-nilai" onclick="tampilNilai()" class="text-blue-600 hover:underline">Lihat Riwayat Nilai</a>
            </div>
        </section>
        ...
        <section id="halaman-nilai" class="bg-white rounded-xl shadow p-6">
            <h2 class="text-xl font-semibold mb-4">📋 Riwayat Hasil Latihan</h2>
            <div id="daftarNilai"></div>
            <button onclick="hapusSemua()" class="mt-4 w-full bg-red-50 text-red-600 border border-red-200 py-2 rounded hover:bg-red-100">🗑️ Hapus Semua Riwayat</button>
        </section>
    </div>
    ...
    <script>
        ...
        const KODE_AKSES = "QRTGHS-2026"; // Kode utama di sini, bisa diubah bebas
        ...
        function prosesDaftar(){
            const u=document.getElementById('userBaru').value.trim(),p=document.getElementById('passBaru').value.trim(),kd=document.getElementById('kodeAkses').value.trim().toUpperCase();
            if(kd !== KODE_AKSES) return alert("❌ Kode salah!");
            if(!u||!p)return alert("⚠️ Isi nama pengguna dan sandi!");
            let d=ambilData('akunSiswa');if(d.find(x=>x.user===u))return alert("⚠️ Nama ini sudah dipakai orang lain!");
            d.push({user:u,sandi:p});simpanData('akunSiswa',d);akunSaatIni=u;alert("✅ Akun berhasil dibuat!");mulaiUjian();
        }
        function prosesMasuk(){
            ...
            if(!x)return alert("❌ Nama atau sandi salah / belum terdaftar!");akunSaatIni=u;mulaiUjian();
        }
        function mulaiUjian(){
            document.getElementById('bagianAkun').classList.add('hidden');
        ...
        function tampilkanSoal(){
            ...
            d.forEach((s,i)=>{
                ...
                b.className="p-3 bg-slate-50 border rounded-lg";
                b.innerHTML=`<p class="font-medium mb-2">${i+1}. ${s.t}</p><div class="space-y-1 text-sm">${o.map((opt,idx)=>`<label class="flex items-center gap-2 cursor-pointer hover:bg-slate-100 p-1 rounded"><input type="radio" name="soal${i}" value="${opt===s.j?'benar':'salah'}">${String.fromCharCode(65+idx)}. ${opt}</label>`).join('')}</div>`;
                w.appendChild(b);
            });
        }
        ...
        function tampilNilai(){
            ...
            if(!d.length){w.innerHTML=`<p class="text-center text-slate-500 py-4">Belum ada riwayat ujian.</p>`;return;}
            w.innerHTML=`<div class="space-y-2">${d.map(x=>`<div class="p-3 border rounded-lg ${x.nilai>=75?'bg-green-50':'bg-orange-50'}"><div class="flex flex-wrap justify-between font-medium">${x.mapel} • Kelas ${x.kelas} • ${x.tingkat}<strong>${x.nilai}/100</strong></div><div class="text-sm text-slate-600">👤 ${x.nama} | 🕒 ${x.waktu}</div></div>`).join('')}</div>`;
        }
        function hapusSemua(){if(confirm("Yakin ingin menghapus SEMUA riwayat nilai?")){localStorage.removeItem('catatanNilai');tampilNilai();}}
        tampilNilai();
    </script>
</body>
</html>
